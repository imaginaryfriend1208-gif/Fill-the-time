...
Write the entire diary in English. Aim for at least 6000 tokens and at most 20000 tokens; be thorough rather than brief. Return one plaintext block without markdown.
...
<task>Merge the previous diary and the newest events into ONE updated development diary. Always write the entire diary in English, regardless of the story's language. The diary must be thorough: aim for at least 6000 tokens and at most 20000 tokens. Do not leave out events to save space; only compress wording, never content.</task>
<format>
...
<task>Rewrite your diary by merging the previous diary with what just happened, into ONE updated diary. The diary must be thorough: aim for at least 6000 tokens and at most 20000 tokens. Do not leave out events to save space; only compress wording, never content.</task>
<format>
...
export function openRegenerationPopup(proposal, endMsgId) {
    return new Promise(async resolve => {
        ...
        accept.off('.ftt').on('click.ftt', async () => {
            ...
            if (!text) { toastr.warning(getText('rmr_summary_empty', 'Summary cannot be empty.'), 'Fill the Time'); return; }
