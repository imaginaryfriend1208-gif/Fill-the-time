...
You may include both styles in the same array. Return ONLY the JSON array, no code fences or commentary.`,
	...
	"use_chunk_summaries_as_chapter": false, // use chunk summaries directly as the stored chapter summary
...
function getSettingValue(setting_key) {
	if (settings[setting_key] === undefined || settings[setting_key] === null || settings[setting_key] === 'undefined') {
		settings[setting_key] = defaultSettings[setting_key];
		getContext().saveSettingsDebounced();
	}
...
async function loadSettingsUI() {
	...
	$("#rmr_use_chunk_summaries_as_chapter").prop('checked', settings.use_chunk_summaries_as_chapter).on('click', toggleCheckboxSetting);
	...
	$(`.rmr-extension_block input[type="number"]`).each((_i, elem) => {
		...
		if (!(setting_key in defaultSettings)) return;
	...
	$(`.rmr-extension_block textarea`).each((_i, elem) => {
		const setting_key = elem.id.replace('rmr_', '');
		if (!(setting_key in defaultSettings)) return;
		elem.value = getSettingValue(setting_key);
...
async function setManualChapterDefaults(force = false) {
	const startInput = $('#rmr_create_chapter_start');
	const endInput = $('#rmr_create_chapter_end');
	...
	const { getTimelineEntries } = await import('./memories.js');
	const timeline = getTimelineEntries();
	const lastChapterEnd = timeline.length
		? Math.max(...timeline.map(chapter => Number(chapter.endMsgId) || 0))
		: -1;
	const defaultStart = Math.min(Math.max(lastChapterEnd + 1, 0), chat.length - 1);
	const defaultEnd = chat.length - 1;
	const shouldReplace = (value) => value === '' || value === undefined || value === null || value === 'undefined';

	if (force || shouldReplace(startInput.val())) {
		startInput.val(defaultStart);
	}
	if (force || shouldReplace(endInput.val())) {
		endInput.val(defaultEnd);
	}
}

function setupManualChapterControls() {
	const startInput = $('#rmr_create_chapter_start');
	const endInput = $('#rmr_create_chapter_end');
	const createButton = $('#rmr_create_chapter');
	if (!startInput.length || !endInput.length || !createButton.length) return;

	startInput.off('change');
	endInput.off('change');
	setManualChapterDefaults();

	createButton.off('click').on('click', async () => {
		const start = Number(startInput.val());
		const end = Number(endInput.val());
		const { createManualChapter } = await import('./memories.js');
		const result = createManualChapter(start, end, '');

		if (!result.ok) {
			toastr.error(result.message, 'Fill the Time');
			return;
		}

		toastr.success(`${result.message} Paste your summary and press Save.`, 'Fill the Time');
		await renderSummariesList();
		resetMessageButtons();
		await setManualChapterDefaults(true);

		const textarea = $(`.rmr-summary-text[data-chapter="${result.chapterNumber}"]`);
		if (textarea.length) {
...
function refreshPromptFields() {
	...
	$('#rmr_memory_system_prompt').val(getSettingValue('memory_system_prompt'));
	$('#rmr_memory_prompt_template').val(getSettingValue('memory_prompt_template'));
	$('#rmr_chapter_query_system_prompt').val(getSettingValue('chapter_query_system_prompt'));
	$('#rmr_chapter_query_prompt_template').val(getSettingValue('chapter_query_prompt_template'));
	$('#rmr_timeline_fill_system_prompt').val(getSettingValue('timeline_fill_system_prompt'));
	$('#rmr_timeline_fill_prompt_template').val(getSettingValue('timeline_fill_prompt_template'));
...
${settings.keywords_prompt}`;
	...
	for (const key in defaultSettings) {
		if (settings[key] === undefined || settings[key] === null || settings[key] === 'undefined') {
...
export async function importPreset(jsonData) {
	try {
		...
		if (presetType !== 'summarize' && presetType !== 'query' && presetType !== 'timeline_fill') {
...
export function exportAllSettings() {
	...
	const exportData = {
		...
		settings: {
			...
			use_chunk_summaries_as_chapter: settings.use_chunk_summaries_as_chapter,
...
export async function importAllSettings(jsonData) {
	try {
		...
		// Validate import data structure
		if (!importData.extension || !['fill-the-time', 'timeline-memory'].includes(importData.extension)) {
			throw new Error('Invalid export file: not a Fill the Time configuration');
		}
		...
		if (result.importSettings) {
			...
			if (importData.settings.use_chunk_summaries_as_chapter !== undefined) settings.use_chunk_summaries_as_chapter = importData.settings.use_chunk_summaries_as_chapter;
		...
		if (result.importPresets) {
			const presetTypes = ['summarize', 'query', 'timeline_fill'];
...
async function showMasterImportDialog(importData) {
	return new Promise((resolve) => {
		...
		const totalPresets = presetCounts.summarize + presetCounts.query + presetCounts.timeline_fill;

		const message = `Fill the Time Configuration Import\n\n` +
			`This file contains:\n` +
			`• Settings configuration\n` +
			`• ${totalPresets} presets (${presetCounts.summarize} summarize, ${presetCounts.query} query, ${presetCounts.timeline_fill} timeline fill)\n\n` +
...
function handleMasterExport() {
	try {
		...
		const a = document.createElement('a');
		a.href = url;
		a.download = `fill-the-time-config-${new Date().toISOString().split('T')[0]}.json`;
		document.body.appendChild(a);
		a.click();
		document.body.removeChild(a);
		URL.revokeObjectURL(url);
...
function refreshSettingsUI() {
	...
	$('#rmr_use_chunk_summaries_as_chapter').prop('checked', settings.use_chunk_summaries_as_chapter);
...
export async function renderSummariesList() {
	...
	const renderVersion = ++summariesRenderVersion;

	// Dynamically import to avoid circular dependencies
	const { getTimelineEntries, updateChapterSummary, getChapterEffectiveRange, getChapterTokenCount } = await import('./memories.js');
	const timeline = getTimelineEntries();

	if (!timeline || timeline.length === 0) {
		if (renderVersion !== summariesRenderVersion) return;
		container.empty();
		container.append('<div class="rmr-summaries-empty">No chapters in current chat. End a chapter to create a summary.</div>');
		return;
	}

	const chapterMeta = await Promise.all(timeline.map(async (chapter, index) => {
		const chapterNum = index + 1;
		const range = getChapterEffectiveRange(chapterNum);
		const startMsg = range?.startMsgId ?? 0;
		const endMsg = range?.endMsgId ?? chapter.endMsgId;
		const tokenCount = await getChapterTokenCount(chapterNum);
		return { chapter, chapterNum, startMsg, endMsg, tokenCount };
	}));
		...
		const summaryItem = $(`
			<div class="rmr-summary-item" data-chapter="${chapterNum}">
				<div class="rmr-summary-header">
					...
					<span class="rmr-summary-range">Messages ${startMsg} - ${endMsg} | Tokens ${tokenCount}</span>
	...
	// Handle inline remove button clicks
...
async function openSummaryPopup(chapterNum, startMsg, endMsg, currentText, updateChapterSummary) {
