...
async function removeReasoningBlocks(input) {
    ...
    if(!input){toastr.warning('Usage: /remove-reasoning 1-10','IF Memory');return '';}
    if(!Array.isArray(chat)||!chat.length)return '';
    try{
        const range=stringToRange(input.trim(),0,chat.length-1);if(!range){toastr.error('Invalid range.','IF Memory');return '';}
        let removed=0;
        ...
        if(removed){await saveChatConditional();await reloadCurrentChat();toastr.success(`Removed ${removed} reasoning block${removed===1?'':'s'}.`,'IF Memory');}
        else toastr.info('No reasoning blocks found.','IF Memory');
        return `Removed ${removed} reasoning block${removed===1?'':'s'}`;
    }catch(error){debug('Remove reasoning failed:',error);toastr.error('Failed to remove reasoning.','IF Memory');return '';}
}
...
async function removeToolCalls(){
    ...
    if(!remove.size){toastr.info('No tool-call messages found.','IF Memory');return 'No tool-call messages found.';}
    [...remove].sort((a,b)=>b-a).forEach(index=>chat.splice(index,1));await saveChatConditional();await reloadCurrentChat();
    const result=`Removed ${remove.size} tool-call related message${remove.size===1?'':'s'}.`;toastr.success(result,'IF Memory');return result;
}
...
export function loadSlashCommands(){
    ...
    const endCallback=async(args,value)=>{
        ...
        if(!Number.isInteger(id)||id<0||id>=chat.length){toastr.error(`Message ID must be 0-${Math.max(0,chat.length-1)}.`,'IF Memory');return '';}
        if(args.profile){args.profile=profileIdFromName(args.profile);if(!args.profile){toastr.error('Profile not found.','IF Memory');return '';}}
        const bool=value=>value===true||value==='true';
        const hasStock=args.stock!==undefined,hasFresh=args.fresh!==undefined;
        await endChapter(id,{...args,autoAccept:args.autoAccept===true?true:(args.autoAccept===false?false:undefined),useStock:hasStock?bool(args.stock):undefined,ignorePrevious:hasFresh?bool(args.fresh):undefined});return '';
    };
    parser.addCommandObject(Command.fromProps({name:'fillthetime-end',aliases:['chapter-end'],callback:endCallback,unnamedArgumentList:[Arg.fromProps({description:'Message ID; defaults to latest',typeList:[types.NUMBER],isRequired:false,enumProvider:commonEnumProviders.messages()})],namedArgumentList:[Named.fromProps({name:'profile',description:'Connection profile name',enumProvider:profilesProvider,isRequired:false}),Named.fromProps({name:'quiet',description:'Suppress toasts',typeList:[types.BOOLEAN],isRequired:false}),Named.fromProps({name:'autoAccept',description:'Silent background merge: auto-apply without the review popup',typeList:[types.BOOLEAN],isRequired:false}),Named.fromProps({name:'stock',description:'Reuse stocked chunks (false = read raw messages)',typeList:[types.BOOLEAN],isRequired:false}),Named.fromProps({name:'fresh',description:'Fresh merge: ignore the previous summary and the archive',typeList:[types.BOOLEAN],isRequired:false})],helpString:'Generate and review the rolling summary through a message.'}));
    ...
    parser.addCommandObject(Command.fromProps({name:'fillthetime-archive-hide',callback:async(args,value)=>{const{isArchiveIsolated,setArchiveIsolated}=await import('./memories.js');const want=value===''||value==null?!isArchiveIsolated():(value===true||value==='true');await setArchiveIsolated(want);return String(want);},unnamedArgumentList:[Arg.fromProps({description:'true/false; blank toggles',typeList:[types.BOOLEAN],isRequired:false})],helpString:'Hide or show the per-chat archive (hidden archives are ignored by merge and regeneration).'}));
    parser.addCommandObject(Command.fromProps({name:'fillthetime-archive-clear',callback:async()=>{const{clearArchiveEntries}=await import('./memories.js');return String(await clearArchiveEntries());},helpString:'Permanently delete every archived summary of this chat.'}));
    parser.addCommandObject(Command.fromProps({name:'fillthetime-stock-clear',callback:async()=>{const{clearStockedChunks}=await import('./memories.js');return String(await clearStockedChunks());},helpString:'Delete every stocked chunk summary of this chat.'}));
